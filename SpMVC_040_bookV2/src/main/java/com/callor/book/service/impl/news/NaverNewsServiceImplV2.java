 ...
 *  OpenAPI를 통하여 JsonString을 수신하고, parsing을 통해
 *  DTO, List<DTO> 형태의 데이터로 변환하는 과정을 수행했다
 *  
 * RestTemplate을 사용하면
...
public class NaverNewsServiceImplV2 extends NaverNewsServiceImplV1{
	...
	public List<NewsDTO> getNaverList(String queryURL) throws Exception {
		...
		// RestTemplate을 통해 수신된 데이터를
		// 자동 Parsing하기 위한 클래스
		ResponseEntity<NaverParentDTO> resList = null;
		
		// Spring framework 3.2에서 처음 도입된 클래스
		// OpenAPI에 요청하기 위한 클래스
		RestTemplate restTemp = new RestTemplate();
		
		resList = restTemp.exchange(restURT, // naver에 요청할 URL 정보 
									HttpMethod.GET, // GET method로 요청
									entity, // 인증정보 등이 담긴 header
		...
		List<NewsDTO> news = resList.getBody().items;
