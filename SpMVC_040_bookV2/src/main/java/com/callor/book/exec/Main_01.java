...
public class Main_01 {
	...
	public static void main(String[] args) {
		...
		// 1.빈(blank) 객체를 생성하고, 필요한 값을 setter를 사용하여 저장하기
		//   	단점: 필요한 데이터만큼 Setter method를 계속 나열해야 한다
		BookDTO bookDTO = new BookDTO();
		bookDTO.setTitle("자바야놀자");
		
		// 2. 필드생성자를 사용하여
		//		객체를 생성하면서, 값을 주입하는 방법
		//		단점 : - 일부 데이터를 위하여 별도로 생성자를 또 만들어야 한다
		// 			   - 데이터의 주입 순서가 바뀌면 어떤 값이 저장될지 모른다 전적으로 개발자 책임으로 귀결된다	
 		BookDTO bookDTO1 = new BookDTO("자바야놀자", "linke", "image",
										"author","price","discount",
										"publisher","isbn","desc","pubdate");
 		...
 		//		 객체 인스턴스(bookDTO2)에게 전달하기 위하여
 		//		 build() method를 호출한다
 		
 		//	장점 :
 		//	 	new 키워드로 생성자를 호출하지 않는다
 		//		필요한 변수(속성,멤버변수)만 가지고 있는 객체를 만들수 있다
