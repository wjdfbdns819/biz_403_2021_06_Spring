...
public class StudentServiceImplV1 implements StudentService{
	...
	public String makeStNum(String prefix) {
		...
		/*
		 * prefix 만큼의 문자열을 건너뛰고, 나머지 부분 추출하기
		 *  	ex) stNum = "20210010"이고,
		 *			 prefix = "2021"이라면
		 *			 stSeq = stNum.subString(4) 이런 형식의 코드가 생성되어
		 *			 stSeq에는 0010d의 문자열만 남게된다  	  
		 */
		...
		log.debug("새로 생성된 학번 seq {}", newStNum);
	 ...
	 *   문제가 발생하고, 데이터의 오류가 저장될 것이다
	 *   이런 상황을 방지 하기 위하여 업무 단위를 transaction이라는
	 *   단위로 묶고,
	 *   
	...
	public String scoreInput(ScoreInputVO scInputVO) {
		 ...
		 *  
		 *   그랬더니 transactionManager에 의해
		 *   모든 Insert Or Update가 Rollback되어 버렸다
		 */
		
		// 이유 불문하고 exception을 발생하라
