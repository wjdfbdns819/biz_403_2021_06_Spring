...
public class HomeController {
	...
	//localhost:8080/rootPath/dumy/gallery/detail 요청했을때
	// Request를 처리할 method
	// a tag를 클릭했을때 <a href="${rootPath}/dumy/gallery/detail">
	 ...
	 * 	<button type ="submit"
	 * 
	 * JSP, HTML에서 위코드를 만들고 입력화면을 보여준후
	 * 	input box에 어떤 문자열을 입력한 후
	 * 	전송 button을 클릭하면
	 * 이 method가 Request를 수신하고 
	 *  input box에 입력한 문자열은 str 변수에 담기기 된다
	 */
	...
	public String home(@RequestParam("one_file") MultipartFile one_file, 
		 ...
		 *  input tag의 name 값을 지정해 준다
		 *  또한 input tag에는 mulitple = "multiple"이 설정되어있어야 한다
		 */
		List<MultipartFile> files = m_file.getFiles("m_file");
